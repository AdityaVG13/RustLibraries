...
| Benchmark ranking | Self-authored targeted suite: NumRust wins 8 of 8 vs NumPy 2.4.4, 1.53x geomean speedup. Externally derived NumPy ASV suite: NumRust wins 52 of 53 supported cases, NumPy wins 1, 9.95x geomean speedup, 1 near-tie within 2%, and is ranked higher by wins on supported external cases. |
...
- External NumPy ASV-derived comparison: NumRust 52 wins, NumPy 1 win, 9.95x geomean speedup, 1 near-tie within 2%, ranked higher by wins on supported cases; 1 unsupported external case bucket tracked.
...
This is not globally better than NumPy. It is measurably better on the targeted self-authored suite, currently ranks higher by wins on supported external ASV-derived cases, and now passes the pinned upstream Array API 2023.12 suite. The latest external full run has 1 NumPy-winning near-tie case, `asv_linalg_einsum_scalar_mul_f64_480000`, and the focused rerun flips that case to NumRust. It still does not match NumPy's full API surface, NumPy ABI, mature release ecosystem, optimized native decomposition backends, or years of real-world production battle testing.

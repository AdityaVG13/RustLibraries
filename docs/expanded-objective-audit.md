...
| Benchmark ranking | Targeted same-data suite: NumRust wins 10 of 10 vs NumPy 2.4.4, 1.67x geomean speedup. Externally derived NumPy ASV suite: NumRust wins 101 of 102 supported cases, NumPy wins 1 linalg near tie, 17.47x geomean speedup, 7 near-ties within 2%, and is ranked higher by wins on supported external cases. |
...
- External NumPy ASV-derived comparison: NumRust 101 wins, NumPy 1 win, 17.47x geomean speedup, 7 near-ties within 2%, ranked higher by wins on supported cases; 1 unsupported external case bucket tracked.
...
This is not globally better than NumPy. It is measurably better on the targeted same-data suite, currently ranks higher by wins on supported external ASV-derived cases, and now passes the pinned upstream Array API 2023.12 suite. The latest external full run still has 1 NumPy-winning supported linalg near tie, and the project does not match NumPy's full API surface, NumPy ABI, mature release ecosystem, optimized native decomposition backends, or years of real-world production battle testing.

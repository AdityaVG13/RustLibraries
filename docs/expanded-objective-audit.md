...
| Benchmark ranking | Targeted same-data suite: NumRust wins 10 of 10 vs NumPy 2.4.4, 1.67x geomean speedup. Externally derived NumPy ASV suite: NumRust wins 65 of 67 supported cases, NumPy wins 2, 8.47x geomean speedup, 4 near-ties within 2%, and is ranked higher by wins on supported external cases. |
...
- External NumPy ASV-derived comparison: NumRust 65 wins, NumPy 2 wins, 8.47x geomean speedup, 4 near-ties within 2%, ranked higher by wins on supported cases; 1 unsupported external case bucket tracked.
...
This is not globally better than NumPy. It is measurably better on the targeted same-data suite, currently ranks higher by wins on supported external ASV-derived cases, and now passes the pinned upstream Array API 2023.12 suite. The latest external full run has 2 NumPy-winning rows, `asv_linalg_matmul_trans_a_at_f64_150x400_400x150` and `asv_linalg_matmul_trans_atc_a_f64_400x150_150x400`; the focused rerun flips both to NumRust but remains a focused triage signal. It still does not match NumPy's full API surface, NumPy ABI, mature release ecosystem, optimized native decomposition backends, or years of real-world production battle testing.

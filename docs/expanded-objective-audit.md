...
| Scope expansion | `numrs-core` includes views, shape/strides, uniform-array metadata, slicing, reshape, transpose, ravel, expand/squeeze dims, broadcasting, concatenate, stack, reductions, stats reductions, arg reductions, selected einsum-style contractions including bilinear forms, dot, 2-D inner, matmul, tensordot axes, small linalg, in-place ops, fused add+sum, indexed writes, scatter-at updates, and fancy indexing foundations. `numrust-python` adds the first Rust-backed Python namespace bridge. |
...
| Benchmark ranking | Targeted same-data suite: NumRust wins 10 of 10 vs NumPy 2.4.4, 1.67x geomean speedup. Externally derived NumPy ASV suite: NumRust wins 61 of 62 supported cases, NumPy wins 1, 8.06x geomean speedup, 5 near-ties within 2%, and is ranked higher by wins on supported external cases. |
...
- External NumPy ASV-derived comparison: NumRust 61 wins, NumPy 1 win, 8.06x geomean speedup, 5 near-ties within 2%, ranked higher by wins on supported cases; 1 unsupported external case bucket tracked.

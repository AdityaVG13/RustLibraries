...
| NumPy core | `numrs-core` | NumPy 2.4.4 | Pinned NumPy ASV-derived suite | 108 supported | 108 | 0 | No | 22.07x geomean | 0 | [`external-numpy-asv-inspired.md`](../benchmark-results/external-numpy-asv-inspired.md) |
...
The authoritative full report currently has no NumPy-winning supported rows and 6 linalg near ties. The focused-loss artifact records 0 current loss rows because there are no supported NumPy-winning rows to rerun. The full report remains authoritative, and the global NumPy replacement claim stays false because the supported external slice is not the full NumPy API surface.

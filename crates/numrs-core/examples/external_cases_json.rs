...
fn edge_sum_f32(array: &Array<f32>) -> f64 {
    let values = array.as_slice();
    (values[0] + values[values.len() - 1]) as f64
}

fn edge_sum_i32(array: &Array<i32>) -> f64 {
    let values = array.as_slice();
    (values[0] + values[values.len() - 1]) as f64
}
...
fn main() -> numrs_core::Result<()> {
    ...
    let stats_data_i64 = Array::full(vec![200], 1_i64)?;
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_i64.min_all().unwrap() as f64;
            }
            checksum
        },
        7,
    ...
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_i64.max_all().unwrap() as f64;
            }
            checksum
        },
        7,
    ...
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_i64.mean_all().unwrap();
            }
            checksum
        },
        7,
    ...
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_i64.std_all().unwrap();
            }
            checksum
        },
        7,
    ...
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_i64.prod_all().unwrap() as f64;
            }
            checksum
        },
        7,
    ...
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_i64.var_all().unwrap();
            }
            checksum
        },
        7,
    ...
    let stats_data_u64 = Array::full(vec![200], 1_u64)?;
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_u64.min_all().unwrap() as f64;
            }
            checksum
        },
        7,
    ...
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_u64.max_all().unwrap() as f64;
            }
            checksum
        },
        7,
    ...
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_u64.mean_all().unwrap();
            }
            checksum
        },
        7,
    ...
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_u64.std_all().unwrap();
            }
            checksum
        },
        7,
    ...
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_u64.prod_all().unwrap() as f64;
            }
            checksum
        },
        7,
    ...
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..100_000 {
                checksum += stats_data_u64.var_all().unwrap();
            }
            checksum
        },
        7,
    ...
    for (name, rows, cols, repetitions) in [
        (
        ...
        let source = Array::from_shape_fn(vec![rows, cols], |idx| (idx[0] * cols + idx[1]) as f64)?;
        let scalar_one = Array::full(vec![1], 1.0_f64)?;
        let (millis, checksum) = bench(
            || {
                let mut checksum = 0.0;
    ...
    for (name, size) in [
        ...
        ("asv_manipulate_broadcast_to_f64_512", 512),
    ] {
        let source = Array::from_shape_fn(vec![size], |idx| idx[0] as f64)?;
        let (millis, checksum) = bench(
            || {
                let mut checksum = 0.0;
                for _ in 0..200_000 {
                    let out = source.broadcast_to(&[size, size]).unwrap();
    ...
    let concat_arrays_f32 = (0..5)
        .map(|array_idx| {
            Array::from_shape_fn(vec![32, 64], |idx| {
                (array_idx * 32 * 64 + idx[0] * 64 + idx[1]) as f32
            })
        })
        .collect::<numrs_core::Result<Vec<_>>>()?;
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..2_000 {
                let out = Array::concatenate(&concat_arrays_f32, 0).unwrap();
    ...
    let concat_arrays_i32 = (0..5)
        .map(|array_idx| {
            Array::from_shape_fn(vec![32, 64], |idx| {
                (array_idx * 32 * 64 + idx[0] * 64 + idx[1]) as i32
            })
        })
        .collect::<numrs_core::Result<Vec<_>>>()?;
    let (millis, checksum) = bench(
        || {
            let mut checksum = 0.0;
            for _ in 0..2_000 {
                let out = Array::concatenate(&concat_arrays_i32, 0).unwrap();

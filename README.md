...
- [x] 2026-05-07: Add pinned NumPy ASV `float32` and `int32` `ConcatenateStackArrays` axis-1 owned-output rows for `concatenate` and `stack`. Latest external ASV evidence: 82 NumRust wins, 2 NumPy wins, 8.41x geomean, 0 checksum failures; focused-loss evidence flips both current NumPy-winning rows to NumRust. Evidence: `benchmarks/external_numpy_cases.py`, `benchmark-results/external-numpy-asv-inspired.md`, `benchmark-results/external-numpy-loss-focused.md`.
...
- [ ] 2026-05-07: Stabilize current full external NumPy linalg near ties without hiding rows. Latest full report has 2 NumPy-winning linalg near ties and 7 near-tie rows; the focused 3-pass rerun flips both current NumPy-winning rows to NumRust, but full-report wins remain the target. Target: `crates/numrs-core/src/blas.rs`, `benchmark-results/external-numpy-asv-inspired.md`.

...
- [x] 2026-05-07: Add NumRust `int64` all-axis `mean`, `var`, and `std` support plus pinned NumPy ASV `StatsReductions(dtype=int64)` rows for min, max, mean, std, prod, and var. Latest external ASV evidence: 95 NumRust wins, 1 NumPy win, 14.03x geomean, 0 checksum failures; focused-loss evidence flips the current NumPy-winning row to NumRust. Evidence: `crates/numrs-core/src/ops.rs`, `benchmarks/external_numpy_cases.py`, `benchmark-results/external-numpy-asv-inspired.md`, `benchmark-results/external-numpy-loss-focused.md`.
...
- [ ] 2026-05-07: Stabilize current full external NumPy linalg near ties without hiding rows. Latest full report has 1 NumPy-winning linalg near tie and 5 near-tie rows; the focused 3-pass rerun flips the current NumPy-winning row to NumRust, but full-report wins remain the target. Target: `crates/numrs-core/src/blas.rs`, `benchmark-results/external-numpy-asv-inspired.md`.

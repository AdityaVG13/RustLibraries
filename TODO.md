...
- [x] NumRust integer stats dtype expansion: add `int64` all-axis `mean`, `var`, and `std` support plus pinned `StatsReductions(dtype=int64)` rows for min, max, mean, std, prod, and var. Latest external evidence: 95 NumRust wins, 1 NumPy win, 14.03x geomean, 0 checksum failures; focused-loss evidence flips the current NumPy-winning row to NumRust.
- [ ] NumRust external linalg stability cleanup: stabilize current full-report near ties without hiding rows. Latest full report has 1 NumPy-winning linalg near tie and 5 near-tie rows; the focused 3-pass rerun flips the current NumPy-winning row to NumRust, but full-report wins remain the target.

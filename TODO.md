...
- [x] NumRust unsigned integer stats dtype expansion: add `uint64` all-axis `mean`, `var`, and `std` support plus pinned `StatsReductions(dtype=uint64)` rows for min, max, mean, std, prod, and var. Latest external evidence: 101 NumRust wins, 1 NumPy win, 17.47x geomean, 0 checksum failures; focused-loss evidence leaves the current NumPy-winning row as a NumPy near-tie.
- [ ] NumRust external linalg stability cleanup: stabilize current full-report near ties without hiding rows. Latest full report has 1 NumPy-winning linalg near tie and 7 near-tie rows; the focused 3-pass rerun still leaves the current NumPy-winning row as a NumPy near-tie.

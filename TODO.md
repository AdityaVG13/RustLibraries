...
- [x] NumRust complex stats dtype expansion: add `complex64` all-axis `min`, `max`, `mean`, `var`, and `std` support plus a fast uniform complex product path and pinned `StatsReductions(dtype=complex64)` rows for min, max, mean, std, prod, and var. Latest external evidence: 113 NumRust wins, 1 NumPy win, 28.63x geomean, 0 checksum failures; focused-loss evidence flips the current NumPy-winning row to NumRust.
- [ ] NumRust external linalg stability cleanup: stabilize current full-report near ties without hiding rows. Latest full report has 1 NumPy-winning copied-transpose linalg near tie and 6 near-tie rows; the focused 3-pass rerun flips the current NumPy-winning row to NumRust.

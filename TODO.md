...
- [x] NumRust bool stats dtype expansion: add `bool` all-axis `mean`, `var`, `std`, and `prod` support plus pinned `StatsReductions(dtype=bool_)` rows for min, max, mean, std, prod, and var. Step evidence: 108 NumRust wins, 0 NumPy wins, 22.07x geomean, 0 checksum failures; focused-loss evidence has 0 current loss rows.
- [x] NumRust complex stats dtype expansion: add `complex64` all-axis `min`, `max`, `mean`, `var`, and `std` support plus a fast uniform complex product path and pinned `StatsReductions(dtype=complex64)` rows for min, max, mean, std, prod, and var. Step evidence: 113 NumRust wins, 1 NumPy win, 28.63x geomean, 0 checksum failures; focused-loss evidence flips the current NumPy-winning row to NumRust.
- [x] NumRust FMinMax reduction expansion: add NaN-aware `float32`/`float64` `fmin_all` and `fmax_all` support plus pinned `FMinMax` rows for `np.fmin.reduce` and `np.fmax.reduce`. Latest external evidence: 117 NumRust wins, 1 NumPy win, 32.62x geomean, 0 checksum failures; focused-loss evidence flips the current NumPy-winning row to NumRust.
- [ ] NumRust external linalg stability cleanup: stabilize current full-report near ties without hiding rows. Latest full report has 1 NumPy-winning linalg near tie and 5 near-tie rows; the focused 3-pass rerun flips the current NumPy-winning row to NumRust.
